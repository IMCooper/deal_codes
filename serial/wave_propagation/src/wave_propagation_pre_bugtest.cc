                                  ...
                                  unsigned int component)
{
    ...
    values(1) = ( p_wave(0)*k_wave(2) - p_wave(2)*k_wave(0) )*std::sin(exponent);
    ...
    values(4) = ( p_wave(2)*k_wave(0) - p_wave(0)*k_wave(2) )*std::cos(exponent);
...
//    double quicktest();
    ...
    // Material Parameters:
//    double param_omega = sqrt(0.06);//0.1; // angular frequency, rads/sec. Set elsewhere later.
    double param_omega = 0.1;//0.1; // angular frequency, rads/sec. Set elsewhere later.
...
//template<int dim>
//double MaxwellProblem<dim>::quicktest()
//{
...
//    
//    std::vector<Tensor<1,dim> > sol_value_list(n_q_points);
////     std::vector<Vector<double> > sol_value_list (n_q_points, Vector<double>(dim));//fe.n_components()));
...
//    Tensor<1,dim> cell_sol_re(dofs_per_cell);
//    Tensor<1,dim> cell_sol_im(dofs_per_cell);
//    
//    std::vector<Vector<double> > exactsol(n_q_points, Vector<double>(fe.n_components()));
////    std::vector<Tensor<1,dim> > exactsol(n_q_points);
////    std::vector<Tensor<1,dim> > exactcurl(n_q_points);
//    
...
//         //access function values within a cell:
//         fe_values[E_re].get_function_values(solution, sol_value_list);
//         exact_solution.vector_value_list(fe_values.get_quadrature_points(), exactsol);
//         // access global dofs from local dofs:
//         cell->get_dof_indices (local_dof_indices);
//         for (unsigned int i=0; i<dofs_per_cell; ++i)
//         {
//             std::cout << i << " " << (unsigned int)local_dof_indices[i] << std::endl;
//         }
//         
//         for (unsigned int q_point=0;q_point<n_q_points;q_point++)
//         {
//             std::cout << sol_value_list[q_point][0] << " " << exactsol[q_point](0) << sol_value_list[q_point][0]-exactsol[q_point](0) <<
//             std::endl;// << " " << sol_value_list[q](1) << " " << sol_value_list[q](2) << std::endl;
//             // Loop over DoFs to calculate solution @ quad point
//             cell_sol_re=0.0;
//             cell_sol_im=0.0;
//             for (unsigned int i=0; i<dofs_per_cell; ++i)
//             {
//                 block_index_i = fe.system_to_block_index(i).first;
//                 // Construct local value & local curl value @ quad point
//                 if (block_index_i==0)
//                 {
//                     cell_sol_re += solution(local_dof_indices[i])*fe_values[E_re].value(i,q_point);
////                     for (unsigned int component=0;component<dim;component++)
////                     {
////                         cell_sol_re(component) += solution(local_dof_indices[i])*fe_values[E_re].value(i,q_point)[component];
////                     }
//                 }
//                 else if (block_index_i==1)
//                 {
//                     cell_sol_im += solution(local_dof_indices[i])*fe_values[E_im].value(i,q_point);
////                     for (unsigned int component=0;component<dim;component++)
////                     {
////                         cell_sol_im(component) += solution(local_dof_indices[i])*fe_values[E_im].value(i,q_point)[component];
////                     }
//                 }
//             }
//             std::cout << cell_sol_re[0] << std::endl;// << " " << sol_value_list[q](1) << " " << sol_value_list[q](2) << std::endl;
//
//             
//
//         }
...
//        diff_per_cell(i) = diff_per_cell_re(i)+diff_per_cell_im(i);
//    }
    ...
    */
    ...
    convergence_table.add_value("Total L2 Error", L2_error);
...
//             }
    ...
    convergence_table.set_precision("Total L2 Error",8);
    convergence_table.set_scientific("Total L2 Error",true);
