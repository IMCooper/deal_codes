...
public:
    ...
    virtual void vector_value_list (const std::vector<Point<dim> > &points,
                                    std::vector<Vector<double> >   &values) const;
    void curl_value(const Point<dim> &p,
                                        ...
                                        Vector<double> &values) const
{
    ...
    for (unsigned int k = 0; k < dim; k++){
      ...
      values(k+dim) = p_wave(k)*std::sin(exponent);
    }
    */
...
template <int dim>
void WavePropagation::curl_value(const Point<dim> &p,
				 Vector<double> &result)
{
  
}
template <int dim>
void WavePropagation::curl_value_list(const std::vector<Point<dim> > &points,
				      std::vector<Vector<double> >   &values)
{
...
private:
    ...
    // Material Parameters:
    double param_omega = sqrt(0.01); // angular frequency, rads/sec. Set elsewhere later.
...
//    FullMatrix<double> cell_matrix_mass (dofs_per_cell, dofs_per_cell);
    ...
    std::vector<Point<dim> > check_point;
...
//                                              Vector<double>(dim));
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        check_point = fe_values.get_quadrature_points();
        /*
