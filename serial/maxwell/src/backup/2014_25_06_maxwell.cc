...
void MaxwellProblem<dim>::setup_system ()
{
    ...
    std::cout << "n components" << fe.n_components() << std::endl;
...
//    FullMatrix<double> cell_matrix_mass (dofs_per_cell, dofs_per_cell);
    ...
    for (; cell!=endc; ++cell)
    {
        ...
        exact_solution_re.vector_value_list (fe_values.get_quadrature_points(),
                                             rhs_values_re);
        exact_solution_im.vector_value_list (fe_values.get_quadrature_points(),
                                            rhs_values_im);

        for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
        {
            std::cout << rhs_values_re[q_point](0) << " " << rhs_values_re[q_point](1) << " " << rhs_values_re[q_point](2) << " "
            
            << rhs_values_im[q_point](0) << " " << rhs_values_im[q_point](1)  << " " << rhs_values_im[q_point](2) << std::endl;
...
//    const WavePropagation_Im<dim> exact_solution_im;
    ...
    VectorTools::integrate_difference(dof_handler, solution, exact_solution_im,
