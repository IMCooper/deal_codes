                                        ...
                                        Vector<double> &values) const
{
    ...
    jm1=(((2*alpha)/(omega*r))*boost::math::cyl_bessel_j(alpha,omega*r))-boost::math::cyl_bessel_j(alpha+1,omega*r);
    jp1=boost::math::cyl_bessel_j(alpha+1,omega*r);
    dja=0.5*(jm1-jp1);
    jp2=boost::math::cyl_bessel_j(alpha+2,omega*r);
                                      ...
                                      const unsigned int component) const
{
    ...
    for (unsigned int i=0; i<points.size(); ++i)
    {
        ...
        dja=0.5*(jm1-jp1);
        jp2=boost::math::cyl_bessel_j(alpha+2,omega*r);
        jm2=((2*(-1.0+((double)alpha))/(omega*r))*jm1)-ja;
                                 ...
                                 const unsigned int component) const
{
    ...
    switch(component) {
        case 0:
            val = cos(PI*p(0))*sin(PI*p(1)) + bc_constant;
            break;
        case 1:
            val = -sin(PI*p(0))*cos(PI*p(1)) + bc_constant;
            break;
    }
    return val;
...
void MaxwellProblem<dim>::setup_system ()
{
    ...
    VectorTools::project_boundary_values_curl_conforming(dof_handler, 0, BesselSolution<dim>(alpha,omega), 0, constraints);
