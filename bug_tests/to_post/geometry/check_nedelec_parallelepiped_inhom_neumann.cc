/*
 ...
 Neumann BCs = n x (z, x, y)
 Dirichlet BCs = n x (xz, yx, zy).
                                             ...
                                             std::vector<Vector<double> > &value_list) const
{
    ...
    for (unsigned int i=0; i<n_points; ++i)
    {
        ...
        // Real:
        value_list[i](0) = p(0)*p(2) + 1;
        value_list[i](1) = p(1)*p(0) + 1;
        value_list[i](2) = p(2)*p(1) + 1;
    }
}
// END RHS
                                             ...
                                             std::vector<Vector<double> > &value_list) const
{
    ...
    for (unsigned int i=0; i<n_points; ++i)
    {
        const Point<dim> &p = points[i];
        // Real:
        value_list[i](0) = p(0)*p(2);
        value_list[i](1) = p(1)*p(0);
        value_list[i](2) = p(2)*p(1);
    }
}
